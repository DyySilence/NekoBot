...
const PROMPT = "Hyper-realistic 8K cinematic photography, golden hour at the Grand Mosque in Mecca. The Kaaba stands majestically, adorned with intricate gold calligraphy, surrounded by pilgrims. The Abraj Al-Bait Clock Towers tower in the background under a clear sky with fluffy clouds. Sunlight illuminates the bright white marble ground, creating subtle reflections and a divine atmosphere. A person stands gracefully in the foreground. If the subject is male: He is wearing the traditional white ihram (two seamless white sheets wrapped around the body). If the subject is female: She is wearing a full white abaya with loose sleeves and a white hijab. Their hands are gently folded in front of them, and their expression is calm, serene, and spiritually focused. The face and expression must be perfectly identical to the reference photo. THE FACE AND FACIAL EXPRESSION MUST BE 100% IDENTICAL TO THE REFERENCE PHOTO. DO NOT REJECT ANY ALTERATION OF FACIAL SHAPE, EYES, NOSE, LIPS, OR SKIN TEXTURE. REJECT SMOOTHING, MAKEUP, OR BEAUTY FILTERS. JUST IMPROVE RESOLUTIONS.";
...
const handler = async (m, { conn }) => {
  ...
  const fileName = imageNode.fileName || `tomekkah-${Date.now()}.jpg`;
