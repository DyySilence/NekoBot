...
const HITAMKAN_PROMPT = "Change the subject's skin color to black, resulting in a completely black skin tone. Preserve all facial structure and identifying details of the original—including the shape of the eyes, eyebrows, eyelids, nose contour, lips, wrinkles, and bone structure—so the person remains clearly recognizable. Maintain natural depth, shadows, and realistic anatomy. Keep the eyes, teeth, inside the mouth, hair, and clothing exactly as they appear in the original image. High contrast, photorealistic, clean lighting, realistic textures.";
...
const handler = async (m, { conn }) => {
  ...
  if (!mediaObj || mediaObj.type !== "image") {
    return m.reply(
      ...
      `> ${global.prefix}hitamkan\n>\n` +
      `> 🔁 Atau reply gambar dengan:\n` +
      `> ${global.prefix}hitamkan\n>\n` +
      `> 💡 Command ini akan mengubah warna kulit\n` +
      `> orang di foto menjadi hitam secara AI.`
    );
  }

  const imageNode = mediaObj.node;
  if ((imageNode.fileLength || 0) > 20 * 1024 * 1024) {
    return m.reply(`> ❌ *GAMBAR TERLALU BESAR*\n> 📊 Maksimal: 20MB\n> 💡 Kompres gambar dulu sebelum diproses`);
  }
  ...
  const caption =
    `> 🎨 *HASIL HITAMKAN KULIT*\n>\n` +
    `> ✅ Berhasil diproses dengan AI`;
...
handler.description = "Ubah warna kulit orang di foto menjadi hitam menggunakan AI";
